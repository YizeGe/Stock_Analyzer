...
def search_stocks(query: str, limit: int = 10) -> list:
    ...
    if STOCK_NAME_CACHE is None:
        ...
        # 尝试拉实时行情填充价格
        if STOCK_NAME_CACHE:
            symbols = [s["symbol"] for s in STOCK_NAME_CACHE[:50]]
    ...
    for s in STOCK_NAME_CACHE:
        ...
        if q and s.get("name") and q in s["name"]:
            results.append(s)
            seen.add(s["symbol"])
        elif q and s["symbol"].startswith(q):
            results.append(s)
            seen.add(s["symbol"])
...
def _build_common_stock_list():
    """构建常用股票列表——从 SECTOR_LEADERS 等已有数据源提取"""
    # 从 get_hot_stocks 里的 SECTOR_LEADERS 提取
    seen = set()
    stocks = []
    
    # 从 SECTOR_LEADERS 列表提取（已经在 data_api.py 中定义）
    # 因为 SECTOR_LEADERS 是函数内部变量，我们直接引用相同的数据
    leaders = [
        ("银行", "sh600000"), ("银行", "sh600036"), ("银行", "sh601166"),
        ("保险", "sh601318"), ("券商", "sh600837"), ("券商", "sh601211"),
        ("白酒", "sh600519"), ("白酒", "sz000568"), ("食品", "sh603288"),
        ...
        ("苹果链", "sz002475"), ("算力", "sz000977"), ("AI应用", "sh600745"),
        ...
        ("航运", "sh601919"), ("航空", "sh600115"), ("物流", "sh600233"),
        ("港口", "sh601018"),
        ("军工", "sh600893"), ("通信设备", "sz000063"),
        ("运营商", "sh601728"), ("光纤", "sh600487"),
        ("农业", "sh600598"), ("种子", "sh601998"), ("环保", "sh601330"),
        ("机器人", "sz300024"), ("数据要素", "sh600536"), ("跨境电商", "sz002491"),
        # extra common ones
        ("白酒", "sz000858"), ("家电", "sz000651"), ("家电", "sz000333"),
        ("科技", "sz000725"), ("医药", "sz300760"),
        ("金融", "sh601398"), ("金融", "sh600016"), ("金融", "sh601318"),
        ("新能源", "sz300750"), ("地产", "sz000001"),
        ("芯片", "sh688256"), ("芯片", "sh600703"),
        ("通信", "sz002230"), ("电子", "sh603501"),
        ("AI", "sh601138"),
        # sinced searched
        ("", "sz300059"), ("", "sh600585"), ("", "sh600887"),
        ("", "sh600104"), ("", "sh600019"), ("", "sh600862"),
        ("", "sh601179"), ("", "sh601100"),
    ]

    for sec, code in leaders:
        sym = code[2:] if code.startswith(("sh", "sz")) else code
        if sym not in seen and len(sym) == 6:
            seen.add(sym)
            stocks.append({"symbol": sym, "name": "", "price": 0, "change": 0})

    # 用腾讯批量接口拿名称
    if stocks:
        codes = ",".join(l[1] for l in leaders)
        try:
            SESSION = _requests.Session()
